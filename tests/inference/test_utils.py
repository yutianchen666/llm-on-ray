...
def test_stopping_criteria_sub():
    ...
    assert stopping_criteria(input_ids, scores) == True

    input_ids = torch.tensor([[1, 2, 4]])
    assert stopping_criteria(input_ids, scores) == False
...
def test_is_cpu_without_ipex(mock_infer_conf):
    assert is_cpu_without_ipex(mock_infer_conf) == True

    mock_infer_conf.ipex.enabled = True
    assert is_cpu_without_ipex(mock_infer_conf) == False
